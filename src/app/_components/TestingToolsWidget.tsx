...
export default function TestingToolsWidget({
  ...
  return (
    <div
      className="fixed right-0 top-20 z-50 transition-all duration-300 ease-in-out"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
      style={{
        transform: isExpanded ? "translateX(0)" : "translateX(260px)",
        maxWidth: "calc(100vw - 1rem)",
      }}
    >
      {/* Content panel */}
      <div className="relative w-80 max-w-[calc(100vw-2rem)] max-h-[calc(100vh-6rem)] bg-gray-800/95 backdrop-blur-sm rounded-l-2xl shadow-xl border-l border-gray-600 overflow-visible">
        {/* Handle - positioned on the left side */}
        <div className="absolute left-0 top-1/2 transform -translate-y-1/2 -translate-x-1/2 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold py-4 px-3 rounded-l-2xl shadow-lg cursor-pointer border-2 border-white z-10">
          <div className="writing-mode-vertical text-center text-sm">
            TESTING TOOLS
          </div>
        </div>
        <div className="p-6 text-white h-full overflow-y-auto overflow-x-hidden">
